...
def _normalize_frame(frame: np.ndarray) -> np.ndarray:
    ...
    Steps:
    1. Center coordinates using wrist as origin.
    ...
    """
    centered = frame.copy()
    wrist = centered[WRIST_INDEX:WRIST_INDEX + 1, :]
    centered = centered - wrist
...
def normalize_sequence(sequence: np.ndarray) -> np.ndarray:
    ...
    for frame in reshaped:
        normalized_frames.append(_normalize_frame(frame))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input-csv", type=str, default="output/dataset.csv", help="Path to input CSV.")
    parser.add_argument("--output-csv", type=str, default="output/dataset_normalized.csv", help="Path to save normalized CSV.")
...
def main() -> None:
    args = parse_args()
    out = normalize_csv(args.input_csv, args.output_csv, timesteps=args.timesteps)
